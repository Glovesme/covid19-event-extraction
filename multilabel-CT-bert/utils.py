...
def get_TP_FP_FN(data, prediction_scores, THRESHOLD=0.0):
	predicted_chunks_for_each_instance = dict()
	for (text, chunk, chunk_id, chunk_start_text_id, chunk_end_text_id, tokenized_tweet, tokenized_tweet_with_masked_chunk, gold_chunk, label), prediction_score in zip(data, prediction_scores):
		original_text = text
		# print(text)
...
def get_pairs_v1(data, subtask_list):

	data_dict = {subtask : [] for subtask in subtask_list}
	pairs_dict = {subtask : [] for subtask in subtask_list}
	pairs_list = list()

	for instance in data:
		chunk = instance[1]
		tokenized_tweet_with_masked_chunk = instance[6]
		for key, value in instance[7].items():
			label = value[1]
			data_dict[key].append([chunk, tokenized_tweet_with_masked_chunk, label])

	# generate pairs
	for subtask, instance in data_dict.items():
		for i in range(len(instance)):
			if instance[i][2] == 1:
				for j in range(len(instance)):
					if i!=j:
						pairs_dict[subtask].append((instance[i], instance[j]))
						# pairs_list.append([instance[i], instance[j], subtask])

	return pairs_dict


def get_pairs(data, ratio=None):
	pairs_list = []
	for i in range(len(data)):
		if data[i][8] == 1:
			if ratio is None or ratio >= len(data):
				for j in range(len(data)):
					if i != j:
						pos_sen = (data[i][1], data[i][6])
						neg_sen = (data[j][1], data[j][6])
						pairs_list.append((pos_sen, neg_sen))
			else:
				random_indices_list = random.sample(range(len(data)), ratio+1)
				for j in range(ratio):
					if i != j:
						pos_sen = (data[i][1], data[i][6])
						neg_sen = (data[random_indices_list[j]][1], data[random_indices_list[j]][6])
						pairs_list.append((pos_sen, neg_sen))
					else:
						pos_sen = (data[i][1], data[i][6])
						neg_sen = (data[random_indices_list[-1]][1], data[random_indices_list[-1]][6])
						pairs_list.append((pos_sen, neg_sen))
	return pairs_list
...
def generate_instances(file_name):
    ...
    # instances_list = [[chunk_tweet_tokenized[0], chunk_tweet_tokenized[1], label, chunk_tweet_tokenized[2]] for
    #                   chunk_tweet_tokenized, label in instances_dict.items()]
    # save_file_name = 'pickle_file/' + file_name[:-3].split('/')[-1] + 'pkl'
    # save_in_pickle(instances_dict, save_file_name)

    return instances_list, label_dict


if __name__ == '__main__':
	task_instances_dict, tag_statistics, question_keys_and_tags = load_from_pickle('test_positive.pkl')
	...
	# pairs_dict_v1 = get_pairs_v1(train_data, subtasks_list)
	# for key, pairs in pairs_dict_v1.items():
	# 	print("slot: {}, size: {}".format(key, len(pairs)))
	# print(len(pairs_dict_v1))
	# print('done')
	data = extract_instances_for_current_subtask(task_instances_dict, 'relation')
	train_data, dev_data, test_data = split_instances_in_train_dev_test(data)
	pair_list = get_pairs(train_data, 10)


	# task_instances_dict, tag_statistics, question_keys_and_tags = load_from_pickle('./data/test_positive.pkl')
	# data = extract_instances_for_current_subtask(task_instances_dict, 'name')
	# train_data, dev_data, test_data = split_instances_in_train_dev_test(data)
	# train_pairs = get_pairs(train_data)


	# instances_list, label_dict = generate_instances('./data/test_positive.pkl')
	print('done')
